...
- **Usage:** `em crossdev [FLAGS]`

Set up a cross-compilation target (sysroot + overlay) — crossdev workalike

## Flags
- **`-L --llvm`** — Deliberately no [`RootArg`]: none of `crossdev`'s three actions (`--init-target`/`--setup`/`--show-target-cfg`) read `--root` — it is a parse error after the applet, and a try_into reject in prefix position. Use the LLVM/Clang model (`cross_llvm-*`: host clang cross-targets, no per-target compiler)
...
- **`--privilege <PRIVILEGE>`** — Privilege backend for this crossdev run
