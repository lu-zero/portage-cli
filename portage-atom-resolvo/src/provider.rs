...
impl PortageDependencyProvider {
    ...
    /// The resolvo condition that holds when `flag` is enabled, if the flag
    /// has a solver-decided virtual.
    ...
    /// The resolvo condition that holds when `flag` is disabled, if the flag
    /// has a solver-decided virtual.
