...
| Item | When | Notes |
|------|------|--------|
| `em -p`/`-t` reported wrong USE_EXPAND flags (e.g. `L10N`) for a package whose ebuild `+`-defaults every possible value (`chromium-2.eclass`'s vscode-style `IUSE="+l10n_${lang}"`) | 2026-08-18 | Root cause: once a `USE_EXPAND` variable is active, PMS makes membership in its own expansion authoritative over the flag's own `+`/`-` IUSE default — `em` had no such override. Fixed at the two duplicated `iuse_defaults` builders (`portage-resolve`'s `repo.rs`/`effective_use.rs`), not in `resolve_effective_use` itself. `ResolvePolicy`/`Adapter` gained a `use_expand: &UseExpand` field (built once per resolve, not per package). Live-verified: `L10N="de"` on an `IUSE="+l10n_af +l10n_de"` package now shows `L10N="de -af"`. `9d0ef32`. |
