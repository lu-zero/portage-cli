...
- Two real, pre-existing `--local` bootstrap bugs found 2026-08-23 by the first genuine full real (non-pretend, `--jobs 48`) toolchain run, blocking it partway through the `python` step (unrelated to the same-day `package.provided` fix): stable `app-arch/unzip-6.0_p29-r2` fails under this host's real `gcc-16.1.1` seed compiler (a genuine upstream/Gentoo gap — the `~arch` `6.0_p31` fixes it, confirmed live, just not reachable from a fresh prefix's default stable keywords) ([[local-bootstrap-unzip-gcc16-prototype]]); a real `--jobs` scheduling gap — a backwards RDEPEND edge (`dev-build/meson-format-array` → `dev-lang/python`) is silently dropped from `build_blockers`, unlike a backwards DEPEND/BDEPEND edge which at least gets recorded into `hard_cycle_edges` — so a BDEPEND consumer of a python-exec-wrapped build tool can start before that tool's own RDEPEND interpreter exists ([[jobs-rdepend-backwards-edge-race]])
