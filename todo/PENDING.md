...
| **8** | **`:*` slot-dep overreach** — `em -tpuD rust` pulls `llvm:22`+`clang:22` where emerge keeps installed `llvm-21.1.8` as `[nomerge]`; `set_prefer_newest_slot(deep \|\| …)` suspected. Root cause of the one real llvm conflict em reports. Pairs with the opposite-direction gap in [[deep-in-slot-upgrades]] — measure both row counts together | 🔴 found 2026-07-26 | [[slot-dep-overreach]] |
