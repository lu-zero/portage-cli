...
| **0** | **md5-cache blind spot** — an ebuild with no cache entry was *invisible* to em (reported "no ebuilds"), 63 tree-wide incl. the world member `sys-fs/btrfs-progs-7.1`, which em reinstalled at 7.0 while emerge upgraded to 7.1. Fixed on `md5-cache-fallback`: suspects-only digest (absent or newer-than-sync), memoised in a sync-stamped sidecar; 0.98s → 1.01s. Follow-ups: regen-on-sync, and lazy per-cpv load (memory) | 🟡 fixed, unmerged | [[md5-cache-blind-spot]] |
| **8** | **Finish the update chain** (behind `--complete-graph`, today a dead flag) — `em -tpuD rust` moves `llvm:22`+`clang:22` but leaves installed `lldb` behind, so its `~` pin breaks. Starting the chain is *correct* (deep update); stopping halfway is not. Same root cause as the docutils/sphinx gap: installed dependents' constraints never enter the solve (`provider/mod.rs:744-766`) — one fix, not two | 🔴 found 2026-07-26 | [[slot-chain-completion]] |
| **9** | **`--tree` parity** — emerge's `-t` is the merge list indented by depth (108 `[ebuild` rows on `-tpuD rust`); em's is a bare cpv tree (0 rows), so `-tp` loses everything `-p` shows. `-t` should annotate the plan, not replace it | 🔴 found 2026-07-26 | [[tree-output-parity]] |
