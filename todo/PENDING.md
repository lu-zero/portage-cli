...
|-----|------|--------|--------|
| **1** | **`--prefix`/`--local` should prefer their own toolchain over the host's** — split 2026-07-23 into 3 phases: (1) ✅ `em select` confdir population under `--prefix` — done, found+fixed a real `get_chost` host-fallback bug live in a crossdev-stages sandbox; (2) 🔴 build-path awareness (`shell.rs`'s `chost != cbuild` gate needs broadening); (3) 🔴 new `em active --prefix/--local` command (persistent state + shell-export). `--root` defaulting to host stays correct as-is (catalyst seed-compiler model, not a bug) | 🟡 phase 1/3 done | [[select-toolchain]] |
