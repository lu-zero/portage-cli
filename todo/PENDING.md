...
| **9** | **`--local` bootstrap** — setup ladder done (repo/profile/provided); `toolchain --setup -p` resolves cleanly on Debian 12 and, confirmed 2026-08-23, on this Gentoo/arm64 host too (all 6 steps, `EXIT=0`, no cycle, ends at `sys-devel/gcc`). `package.provided`'s libc-step no-op bug fixed at the root the same day (`InstalledPolicy::Provided` — see `portage-atom-pubgrub`) — the libc step now plans and, live-verified via a real non-pretend `--nodeps sys-libs/glibc` merge, actually builds a real glibc; every other step's plan is byte-for-byte unchanged. A full end-to-end non-pretend run through all 6 steps is still unconfirmed (glibc is a live/git ebuild, too slow to finish in-session) | 🟡 | [[local-bootstrap-provided]], [`local-bootstrap.md`](./local-bootstrap.md), [`local-setup-prereq.md`](./local-setup-prereq.md), [[meson-zstd-python-hard-cycle]] (resolved — real hard cycle, not an em bug) |
...
- Two real, pre-existing `--local` bootstrap bugs found 2026-08-23 by the first genuine full real (non-pretend, `--jobs 48`) toolchain run, blocking it partway through the `python` step (unrelated to the `package.provided` fix): stable `app-arch/unzip-6.0_p29-r2` fails under this host's real `gcc-16.1.1` seed compiler (a genuine upstream/Gentoo gap — the `~arch` `6.0_p31` fixes it, confirmed live, just not reachable from a fresh prefix's default stable keywords), still open ([[local-bootstrap-unzip-gcc16-prototype]]); a genuine irreducible bootstrap cycle (`meson-format-array` ↔ `python` ↔ `zstd`, RDEPEND/RDEPEND/BDEPEND) plus a real soft-cycle-repair starvation defect and a `dependency_graph()` display-graph inconsistency, all fixed 2026-08-24 (`InstalledPolicy::Provided` extended to `python`/`meson-format-array`, `repair_soft_inversions` promotion priority, `--json` gained `order_ok`/`hard_cycle_edges`) ([[jobs-rdepend-backwards-edge-race]]) — confirmed live 2026-08-24 via a real, non-pretend build: `sys-apps/gentoo-functions` (the originally-failing package) now completes successfully. A full real end-to-end bootstrap through all 6 steps still hasn't completed in one run — the unzip bug is still open, and a third, separate, newly-found bug blocks further: `dev-lang/perl` is `TIER1`-provided (borrowed, non-Prefix-aware host perl), so `dev-perl/*` packages under `--local` install their real files under plain `${D}` (host perl's own `Config.pm` paths) instead of the real `${ED}`, leaving `${ED}` never created and `perl_fix_permissions`'s `fperms -R u+w /` dying — precisely root-caused, not yet fixed, needs the same kind of real-EPREFIX-build treatment `dev-lang/python` got today ([[local-perl-installs-outside-eprefix]]). A related, unconfirmed docbook-catalog ordering finding is tracked separately ([[docbook-catalog-rdepend-order]])
- **Systemic bug, ✅ fixed 2026-08-23, architecture reworked same day**: `eapply` had a metadata-extraction-only bash stub that was never unshadowed for real builds — every ebuild's `PATCHES` array silently applied nothing, in every `--local`/`--root`/`--prefix` build all session, with no error. Found while investigating a fresh bug report: real `em --root DIR unzip` failing on `app-arch/bzip2` (unpatched `Makefile-libbz2_so` breaking under `multilib-minimal`'s per-ABI `BUILD_DIR`). First fix was one line (add `eapply` to `init_build_env`'s `unset -f` list); a mechanical audit + a live-confirmed *reverse*-direction bug (a reused shell doing metadata-only work after a real build kept the real, unshadowed `eapply`) led to a full rework: `commands/dual_mode.rs` now registers, per dual-mode name, either the real builtin or a small Rust stub via plain `register_builtin` overwrites — no bash function ever defined for these names, so the shadowing bug class can't recur. Live-reverified: real `--root` unzip merge completes end to end. [[eapply-stub-shadows-real-builtin]]
...
- 🔴 **`em crossdev` needs a GCC version/slot pin flag** (real crossdev's
  `--gcc VERSION`) — found 2026-08-24 auditing whether `em` can replace
  real `crossdev`/`emerge` in `~/Sources/crossdev-stages` (bootable board
  images, separate project). Everything else in that audit (`--ex-pkg`
...
- 🔴 **`has_version` piggybacks `cross-*` atoms off the host, but the
  ...
  `has_version` match for a `cross-*` atom (genuine or, as reproduced
  today via stale sandbox VDB state, stale) can never be safely
