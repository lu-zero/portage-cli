...
mod tests {
    ...
    fn activity_pkg_ctx_emits_phase_enter_leave() {
        ...
        let pkg = ActivityPkgCtx::new(
            ...
            "j".into(),
            None,
            Arc::new("sys-apps/foo-1".parse().unwrap()),
    ...
    fn projection_tracks_inflight_and_completion() {
        ...
        p.apply(&ActivityEvent::PkgStart {
            ...
            job_id: job.into(),
            parent_job_id: None,
            cpv: Arc::new("sys-apps/foo-1".parse().unwrap()),
    ...
    fn remaining_for_eta_remaps_blockers() {
        ...
        p.apply(&ActivityEvent::SessionStart {
            ...
            plan: vec![
                ActivityPlanPkg {
                    cpn: "a/a".parse().unwrap(),
                    cpv: Arc::new("a/a-1".parse().unwrap()),
                    merge_root: ActivityMergeRoot::Target,
                },
                ActivityPlanPkg {
                    cpn: "b/b".parse().unwrap(),
                    cpv: Arc::new("b/b-1".parse().unwrap()),
                    merge_root: ActivityMergeRoot::Target,
                },
                ActivityPlanPkg {
                    cpn: "c/c".parse().unwrap(),
                    cpv: Arc::new("c/c-1".parse().unwrap()),
                    merge_root: ActivityMergeRoot::Target,
                },
            ],
    ...
    fn live_fs_round_trip() {
        ...
        bus.emit(ActivityEvent::SessionStart {
            ...
            plan: vec![ActivityPlanPkg {
                cpn: "app-misc/bar".parse().unwrap(),
                cpv: Arc::new("app-misc/bar-2".parse().unwrap()),
                merge_root: ActivityMergeRoot::Target,
            }],
            blockers: vec![vec![]],
        });
    ...
    fn jsonl_round_trip() {
        let ev = ActivityEvent::PkgEnd {
            ...
            job_id: "j".into(),
            parent_job_id: None,
            cpv: Arc::new("a/b-1".parse().unwrap()),
            cpn: "a/b".parse().unwrap(),
