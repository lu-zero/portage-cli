...
/// derivation branches plus the ABI/libdir probe, `PKG_CONFIG_PATH`
/// sanitization, and the leaked-host-path safety net; drops the
/// uClinux-dist/manual-install/self-patching branches real crossdev needs
/// that don't apply here (this file is regenerated, never hand-installed).
///
/// One divergence from real crossdev: `PKG_CONFIG_LIBDIR` also searches
/// `$ROOT` when it differs from the derived sysroot (`em`'s disposable
/// board-`--root` topology, where sibling packages install progressively
/// into `--root` rather than the shared toolchain sysroot).
...
# A disposable board --root distinct from the toolchain's own sysroot (em's
# board-root topology) has its own, separately-growing set of installed
# .pc files -- search both, not just the sysroot, or a sibling package
# installed into --root (not the shared toolchain) goes unfound.
_root=${ROOT%/}
if [ -n "${_root}" ] && [ "${_root}" != "${PKG_CONFIG_ESYSROOT_DIR}" ]; then
	PKG_CONFIG_LIBDIR="${PKG_CONFIG_LIBDIR}:${_root}/usr/${libdir}/pkgconfig:${_root}/usr/share/pkgconfig"
fi
export PKG_CONFIG_LIBDIR
...
# bug class net-libs/libtirpc hit). --root (above) is a legitimate
# non-sysroot location too, not a leak.
bad_lines=$(
	printf "%s\n" ${output} |
	grep '^-[LI]' |
	grep -v "^..${PKG_CONFIG_ESYSROOT_DIR}${WORKDIR:+\|^..${WORKDIR}}${_root:+\|^..${_root}}"
)
if [ -n "${bad_lines}" ]; then
...
/// pkg-config is real per-target state. Returns `None` if `target`'s
/// symlink isn't there yet, even if some other target under the same root
/// already established a shared script (a target you haven't activated
/// pkg-config for should report as unset, not inherit another's status).
...
mod tests {
    ...
    // Regression for the board-root pkg-config gap: a sibling package
    // installed into a disposable `--root` distinct from the toolchain
    // sysroot must still be found. Real bug: `sys-libs/readline` couldn't
    // locate `ncursesw.pc`, installed under `--root`, because the wrapper
    // only searched the sysroot.
    #[test]
    fn generated_wrapper_searches_both_sysroot_and_distinct_root() {
        let dir = tempfile::tempdir().unwrap();
        let sysroot = dir.path().join("sysroot");
        let root = dir.path().join("board-root");
        std::fs::create_dir_all(sysroot.join("usr/lib64/pkgconfig")).unwrap();
        std::fs::create_dir_all(root.join("usr/lib64/pkgconfig")).unwrap();

        // Echoes PKG_CONFIG_LIBDIR itself rather than forwarding to a real
        // backend -- this test is about what the wrapper searches, not
        // about pkgconf's own resolution.
        let backend = dir.path().join("fake-pkgconf");
        ...
        assert!(
            libdir.contains(sysroot.to_str().unwrap()),
            "must still search the toolchain sysroot: {libdir}"
        );
        assert!(
            libdir.contains(root.to_str().unwrap()),
            "must also search --root when it differs from the sysroot: {libdir}"
        );
    }

    // When ROOT and the derived sysroot are the same path (every topology
    // except em's disposable board-root case), PKG_CONFIG_LIBDIR must not
    // gain a redundant duplicate pair.
    #[test]
    fn generated_wrapper_does_not_duplicate_when_root_equals_sysroot() {
        let dir = tempfile::tempdir().unwrap();
        let sysroot = dir.path().join("sysroot");
        std::fs::create_dir_all(sysroot.join("usr/lib64/pkgconfig")).unwrap();
        ...
        let libdir = String::from_utf8_lossy(&output.stdout);
        assert_eq!(
            libdir.matches(sysroot.to_str().unwrap()).count(),
            2,
            "each of lib*/pkgconfig and share/pkgconfig once, not duplicated: {libdir}"
        );
    }
}
