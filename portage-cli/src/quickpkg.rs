...
async fn run_inner(globals: &Cli, opts: &QuickpkgOpts) -> Result<()> {
    ...
    for arg in &expanded {
        ...
        if pkgs.is_empty() {
            eprintln!("!!! no installed package matches '{arg}'");
        ...
        for pkg in pkgs {
            ...
            match package_one(
                ...
                Err(e) => {
                    eprintln!("!!! Failed to package {cpv}: {e:#}");
                    failed += 1;
                }
            }
        }
    ...
    if !successes.is_empty() {
        ...
        match portage_binpkg::index_pkgdir(&pkgdir, &chost) {
            ...
            Err(e) => eprintln!("warning: could not refresh Packages index: {e:#}"),
        }
    }
...
fn stage_entry(
    ...
    match entry.kind {
        ...
        ContentsKind::Obj => {
            ...
            if !src.exists() {
                eprintln!("warning: missing file (skipped): {src}");
                return Ok(());
            }
        ...
        ContentsKind::Sym => {
            ...
            if target_owned.is_empty() {
                eprintln!("warning: empty symlink target (skipped): {}", entry.path);
                return Ok(());
            }
        ...
        ContentsKind::Fifo | ContentsKind::Dev => {
            ...
            eprintln!("warning: skipping {:?} entry {}", entry.kind, entry.path);
        }
    }
    Ok(())
}
