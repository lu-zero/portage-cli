...
///
/// Marks an event that already carries its own portage-style `">>> "`
/// prefix (`">>> Unpacking …"`, matching real portage's own action lines)
/// — as opposed to an ordinary informational note (real portage's
/// `einfo`). The console formatter (`portage-cli`'s `diag::CompactFormatter`)
/// renders an event on this target bare, and everything else gets
/// `einfo`'s `" * "` marker.
///
